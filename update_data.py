...
TAXONOMY_PATH   = os.path.join(BASE_DIR, 'data', 'method_taxonomy.json')

# ---------- 加载方法分类配置 ----------
def _load_taxonomy():
    """
    读取 method_taxonomy.json，构建两张映射表：
      grandchild_to_parent: {孙类名(小写) -> {key, label}}
      grandchild_to_child:  {孙类名(小写) -> {key, label}}
    同时返回 others_parent: 未知方法归入的父类信息
    """
    path = TAXONOMY_PATH
    # 兼容旧版路径（taxonomy 文件可能与脚本同目录）
    if not os.path.exists(path):
        path = os.path.join(BASE_DIR, 'method_taxonomy.json')
    if not os.path.exists(path):
        print(f"WARNING: 未找到 method_taxonomy.json，方法映射将返回空")
        return {}, {}, {"key": "others", "label": "其他"}

    with open(path, encoding="utf-8") as f:
        taxonomy = json.load(f)

    gc_to_parent = {}   # grandchild -> parent info
    gc_to_child  = {}   # grandchild -> child info
    others_parent = {"key": "others", "label": "其他"}

    for parent in taxonomy.get("parents", []):
        p_info = {"key": parent["key"], "label": parent["label"]}
        if parent["key"] == "others":
            others_parent = p_info
        for child in parent.get("children", []):
            c_info = {"key": child["key"], "label": child["label"]}
            for gc in child.get("grandchildren", []):
                key = gc.strip().lower()
                gc_to_parent[key] = p_info
                gc_to_child[key]  = c_info

    return gc_to_parent, gc_to_child, others_parent

_GC_TO_PARENT, _GC_TO_CHILD, _OTHERS_PARENT = _load_taxonomy()
...
def extract_methods(lines):
    """
    从 method_lines 中匹配孙类关键词，返回:
      detail_found:  匹配到的孙类 key 列表（小写，去重保序）
      parent_found:  对应父类信息列表 [{key, label}, ...]（去重保序）
    """
    detail_found = []
    parent_found = []
    seen_detail  = set()
    ...
    for line in lines:
        ...
        for gc_key, p_info in _GC_TO_PARENT.items():
            if gc_key in line_lower:
                if gc_key not in seen_detail:
                    seen_detail.add(gc_key)
                    detail_found.append(gc_key)
                pk = p_info["key"]
                if pk not in seen_parent:
                    seen_parent.add(pk)
                    parent_found.append(p_info)
    return detail_found, parent_found
...
def merge(sheet1, sheet2, papers):
    ...
    for bid, s1 in sheet1.items():
        ...
        m_detail, m_parents = extract_methods(ml)
        items.append({
            "id": bid, "name": s1["name"],
